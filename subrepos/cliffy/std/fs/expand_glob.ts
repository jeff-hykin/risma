...
// https://deno.land/std@0.196.0/path/separator.ts
var SEP = isWindows ? "\\" : "/";
...
// https://deno.land/std@0.196.0/fs/expand_glob.ts
function split(path3) {
  const s = SEP_PATTERN.source;
  const segments = path3.replace(new RegExp(`^${s}|${s}$`, "g"), "").split(SEP_PATTERN);
  const isAbsolute_ = isAbsolute3(path3);
  return {
    segments,
    isAbsolute: isAbsolute_,
    hasTrailingSep: !!path3.match(new RegExp(`${s}$`)),
...
async function* expandGlob(glob, {
  ...
  const shouldInclude = (path3) => !excludePatterns.some((p) => !!path3.match(p));
  const {
    segments,
