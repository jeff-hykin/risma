...
async function generateTermCount(concepts) {
    ...
    for (const [whichPaper, value] of Object.entries(frequencyMapOf)) {
        for (const [whichTerm, count] of Object.entries(value)) {
            ...
            if (count > 0) {
                paperCountOfTerm[whichTerm] = (paperCountOfTerm[whichTerm] || 0) + 1
            }
        }
    }
    // remove terms that only appear in one paper
    for (const [key, value] of Object.entries(paperCountOfTerm)) {
        if (value<=1) {
            delete totalFreq[key]
        }
    }
    // sort
    totalFreq = Object.fromEntries([...Object.entries(totalFreq)].sort((a,b)=>b[1]-a[1]))
    return {totalFreq, frequencyMapOf}
}
var {totalFreq, frequencyMapOf} = await generateTermCount(structuredClone(concepts))

var conceptNames = Object.keys(totalFreq)
var csvOutput=[]
const columnNames = conceptNames
// csvOutput.push(["Name","Year", ...columnNames])
csvOutput.push(["Name", ...columnNames])
...
for (const key of keys) {
    ...
    csvOutput.push([name, ...columnNames.map(each=>(value[each]||0))])
}
